...
def fun_Caminho():
    caminho = filedialog.askopenfilename(initialdir="/",
                                         title="Select file",
...
# ------Converte as colunas em datatime ------------------------
    df['dtInicio'] = pd.to_datetime(df['dtInicio'].squeeze().tolist(),
                                    format="%d/%m/%Y")
    df['dtFim'] = pd.to_datetime(df['dtFim'].squeeze().tolist(),
                                 format="%d/%m/%Y")
    return df


# Cria uma tabela calendario auxiliar, utilizando a menor data de inicio da
# ausencia e a maior data fim da ausencia
...
class Application(Frame):
    def __init__(self, master=None):
        ...
        self.lb2 = Label()
        self.lb2.configure(text="Qual o nome que deve ser salvo o arquivo?",
        ...
        self.btArquivo.configure(width=10, text="arquivo",
                                 font="Wingdings, 10",
                                 command=lambda: self.fun_inicio())
    ...
    def fun_inicio(self):
        # Chama a função que localiza o caminho do arquivo
        strCaminho = fun_Caminho()
...
# não deixa maximizar e nem alterar o frame
